...
const activations = await db.$queryRaw`SELECT c.code, count(a.id)::int n FROM "ModuleActivation" a JOIN "Company" c ON a."companyId" = c.id GROUP BY 1 ORDER BY 1`
console.log('ACTIVATIONS per company:', activations.map(a => `${a.code}:${a.n}`).join(' | '))
await db.$disconnect()
